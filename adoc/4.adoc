...
Die View erstellen wir als HTML-Datei im template-Ordner.
...
<2> Mit `th:text` geben wir wieder den Text des Elements an. Wir greifen dazu auf den eben deklarierten `article` zu und fragen von diesem den `title` ab.
...
<8> und der Seiten-Text soll die Titel der beiden im Controller erstellten Artikel enthalten
...
In dem Projekt existiert noch der ursprüngliche WelcomeController, welcher auch ein RequestMapping für `/` bereitstellt.
Zwei gleiche Mappings sind nicht möglich - am einfachsten können wir dies lösen, indem wir den alten, nicht mehr gebrauchten Controller und die View löschen.
