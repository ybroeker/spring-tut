...
Anhand der kurzen Beschreibung kann man auch schon direkt einzelne konzeptuelle Klassen TODO erkennen:
...
Die grad entwickelten Klassen können nun implementiert werden. Dazu legen wir ein neues Package an, `domain`, und erstellen darin die Klassen:
...
Außerdem übergeben wir diese nicht dem Kostruktor - ein neuer Artikel hat noch keine Kommentare.

<2> Den Veröffentlichungs-Zeitpunk setzten wir direkt im Konstruktor, anstatt ihn zu übergeben.
...
<1> Auch hier setzen wir den Erstell-Zeitpunk direkt im Konstruktor, anstatt ihn zu übergeben.
