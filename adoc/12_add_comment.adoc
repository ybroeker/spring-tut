...
Im Controller müssen wir eine kleine Anpassung vornehmen sowei eine Methode zum
hinzufügen der Kommentare anlegen.

Zuerst müssen wir beim Anzeigen der Kommentare auch dass Command-Object der View hinzufügen:
...
<1> Diese soll nur auf POST-Request an den Unter-Pfad `comments` einen
