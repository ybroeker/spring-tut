...
Dieses schreib u.a. ein Passwort sowie eine Liste von Rollen (z.B. User, Admin) vor.
...
<3> Außerdem eine Liste von Rollen, diese können einfach als String
(im Format `ROLE_USER`) gespeichert werden.
Die Liste enthält nur einfache Datentypen, man kann sie deshlab mit
`EementCollection` annotieren, damit sie passend in der Datenbank gespeichert werden
...
Da wir den Nutzer geändert haben, müssen wir die Beispieldaten im sql-Skript entsprechend anpassen:
...
Um zu verhinder, dass Passworter im Klartext in der Datenbank stehen, werden diese gehasht -
...
Auf den Pfad `"/"` sollte in diesem Fall jeder Zugriff habem (`permitAll`)
...
Verwenden wir fasche Daten, erscheint eine Fehlermeldung, verwenden wir die in die
