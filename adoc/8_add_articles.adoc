...
in der man den neuen Artikels eingeben kann.
...
Für diese legen wir in `src/main/resources` einen neuen Ordner `article` und in diesem
...
zu `action` auf und wertet dabei die angeben URL aus. Die ist die URL,
...
das zugehörige Feld aus dem hinterlegtem Model - `*{titel}` zb `createArticleCmd.title`
