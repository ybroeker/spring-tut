...
<1> Gibt an, das dieses Projekt eine Spring Boot-Application ist
 Dies aktiviert u.a. Auto-Configuration (Jede im Projekt gefundene Konfiguration wird angewandt) und Component-Scan (Alle im Projekt gefundenen Services werden als Beans bereitgestellt)

<2> Zusätzlich wird in der `main` die Klasse an Spring zum Starten übergeben, ähnlich, wie zB mit JavaFX
...
Die View ist in diesem Fall ein Template, genauer das Template `welcome`, was aufgelöst wird src/main/resources/templates/welcome.html.
Das Model ist besteht aus mehreren Objekten, die in der View über ihren Namen referenzierbar sind.
