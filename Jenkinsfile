    ...
    stages {
        stage('Build') {
            steps {
                ...
                s3Upload consoleLogLevel: 'INFO', dontWaitForConcurrentBuildCompletion: false, entries: [[bucket: 'tdpe.ybroeker.de', excludedFile: '**/.asciidoctor/**', flatten: false, gzipFiles: false, keepForever: false, managedArtifacts: false, noUploadOnFailure: true, selectedRegion: 'eu-central-1', showDirectlyInBrowser: false, sourceFile: 'html/**', storageClass: 'STANDARD', uploadFromSlave: false, useServerSideEncryption: false]], pluginFailureResultConstraint: 'UNSTABLE', profileName: 'spring-boot-tut-publisher', userMetadata: []
            }
        }
    }
}
